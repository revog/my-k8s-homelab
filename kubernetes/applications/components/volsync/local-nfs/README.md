...
Our GitOps pipeline uses a decoupled multi-label system inside the application's `pvc.yaml` to safely separate ongoing backups from the initial volume restoration phase:
* `volsync.backube/backup: "true"` — Enables the cluster-wide `ReplicationSource` for active data protection.
* `volsync.backube/restore: "true"` — Injects the Local NFS `dataSourceRef` populator.
* `volsync.backube/restore-s3: "true"` — Injects the Cloudflare S3 `dataSourceRef` populator.

> [!WARNING]
> Only set a restore label to `"true"` during initial deployment if a valid backup snapshot already exists in the destination repository!
...
1. Configure your application's `pvc.yaml` to **opt-out** of the initial restore:
   ...
   metadata:
     ...
     labels:
       ...
       volsync.backube/restore: "false"    # <-- Prevents the NFS PVC from hanging in Pending
       volsync.backube/restore-s3: "false" # <-- Prevents the S3 PVC from hanging in Pending
   ```
2. Commit and push. The empty volume provisions immediately and the application boots up.
3. Once the app generates its initial data, VolSync will automatically run its first backup schedule based on the components sourced.

---

## 🔄 Scenario B: Disaster Recovery (Trigger a Manual Restore)

If an application or PVC was destroyed and you need to restore your historical data from either the NFS or S3 backup repository without fighting the Flux GitOps reconciliation loop, follow this sequence:

### 1. Configure the Manifests for Recovery
Before pushing to Git, update the application's `pvc.yaml` to enable the matching volume populator engine:

* **For Local NFS Restore:**
  ```yaml
  labels:
    volsync.backube/backup: "true"
    volsync.backube/restore: "true"
    volsync.backube/restore-s3: "false"
  ```
* **For Cloudflare S3 Restore:**
  ```yaml
  labels:
    volsync.backube/backup: "true"
    volsync.backube/restore: "false"
    volsync.backube/restore-s3: "true"
  ```
...
*(Note: If restoring from S3, append `-s3` to your `<RESTORE-NAME>`)*:
```bash
